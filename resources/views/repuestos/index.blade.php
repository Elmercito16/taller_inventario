...
<div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $repuestos->total() ?? $repuestos->count() }}</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-red-600">
                        {{ $repuestos->filter(function($item) { return $item->cantidad <= $item->minimo_stock; })->count() }}
                    </p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600">
                        S/ {{ number_format($repuestos->sum(function($item) { return $item->cantidad * $item->precio_unitario; }), 2) }}
                    </p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-purple-600">
                        {{ $repuestos->pluck('categoria_id')->unique()->count() }}
                    </p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <button class="inline-flex items-center px-4 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors duration-200">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative lg:w-96">
                <form action="{{ route('repuestos.index') }}" method="GET" class="flex">
                    <div class="relative flex-1">
                        ...
                        <input 
                            ...
                            class="block w-full pl-10 pr-10 py-2.5 border border-gray-300 rounded-l-lg focus:ring-primary-500 focus:border-primary-500 bg-white"
                        >
                        @if(request('q'))
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <p class="text-blue-800">
                        Mostrando resultados para: <strong>"{{ request('q') }}"</strong>
                        ({{ $repuestos->count() }} {{ Str::plural('resultado', $repuestos->count()) }})
                    </p>
                </div>
                <a href="{{ route('repuestos.index') }}" 
            ...
            <div class="col-span-full">
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-6 max-w-md mx-auto">
                        @if(request('q'))
                            No se encontraron repuestos que coincidan con tu búsqueda. Intenta con otros términos.
                        @else
    ...
    <!-- Paginación mejorada -->
    @if(!request('q') && $repuestos->hasPages())
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    {{ $repuestos->appends(request()->query())->links() }}
                </div>
            </div>
        </div>
    @endif
    ...
    @php
        $stockBajo = $repuestos->filter(function($item) { 
            return $item->cantidad <= $item->minimo_stock && $item->cantidad > 0; 
        });
        $agotados = $repuestos->filter(function($item) { 
            ...
            @if($agotados->count() > 0)
                <div class="mb-4">
                    <p class="text-red-700 font-medium mb-2">
                        ...
                        {{ $agotados->count() }} {{ Str::plural('repuesto agotado', $agotados->count()) }}:
                    </p>
                    <div class="flex flex-wrap gap-2">
                        @foreach($agotados->take(5) as $item)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                {{ $item->nombre }}
                            </span>
                        @endforeach
            ...
            @if($stockBajo->count() > 0)
                <div>
                    <p class="text-yellow-700 font-medium mb-2">
                        ...
                        {{ $stockBajo->count() }} {{ Str::plural('repuesto con stock bajo', $stockBajo->count()) }}:
                    </p>
                    <div class="flex flex-wrap gap-2">
                        @foreach($stockBajo->take(5) as $item)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
